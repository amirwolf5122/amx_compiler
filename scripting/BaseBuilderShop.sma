...
public plugin_init() 
{
	...
	g_pCvarZombieCost[Item_Bomb_Jump_Z] = register_cvar("bb_cost_Bomb_Jump_zombie", "8000")
...
{ 
	...
	menu_additem(iMenu, Text, "4")
	formatex(Text, charsmax(Text), " \wJump Bomb \y[\r%d $\y]", get_pcvar_num(g_pCvarZombieCost[Item_Bomb_Jump_Z]))
	menu_additem(iMenu, Text, "5")
...
public ZombieShop(id, iMenu, item)
{
	...
	else
	{
		switch(key)
		{
			...
			case Item_Bomb_Jump_Z:
			{
				cs_set_user_money(id, iNewMoney, 1)
				client_cmd(id, "zp_bomb_jumper_Amir_wolf")
			}
